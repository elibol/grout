...
OUT="$ROOT/benchmarks/results/b200_mask_split_eval_20260819"
...
echo 'pp,arm,round,mask_split,prefill_ms,decode_ms,e2e_ms,text_sha256' > "$OUT/results.csv"

run_one() {
    local pp="$1" arm="$2" round="$3" mask variant log text_file
    ...
    if [[ "$arm" == old ]]; then
        env "${common_env[@]}" GROUT_FMHA_PREFILL_LPT_MASK_SPLIT=0 \
            "${cmd[@]}" >"$log" 2>&1
    else
        env -u GROUT_FMHA_PREFILL_LPT_MASK_SPLIT "${common_env[@]}" \
            "${cmd[@]}" >"$log" 2>&1
    fi
    ...
    printf '%s,%s,%s,%s,%s,%s,%s,%s\n' \
        "$pp" "$arm" "$round" "$mask" "$prefill_ms" "$decode_ms" "$e2e_ms" "$text_sha" \
...
for pp in 16384 32768; do
    baseline="$OUT/text/pp${pp}_old_r1.txt"
